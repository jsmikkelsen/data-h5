...
Dette dokument dækker **Del 9 – Test jeres netværk** i opgavebeskrivelsen. Her gennemgås, hvordan netværket testes systematisk, hvad der forventes at ske, og hvilke Cisco IOS/klient-kommandoer der anvendes til at bekræfte resultaterne i din opdaterede Packet Tracer-opsætning med dynamisk OSPFv2.
...
* **Beskrivelse:** En pc tilsluttes en aktiv port i Administrationen (`Switch0`, VLAN 10) og anmoder om en IP-adresse via DHCP.
* **Forventning:** Core Switch Primary (`Multilayer Switch0`) modtager DHCP Discover-pakken direkte på sit SVI (VLAN 10). Den tildeler en IP-adresse fra sin primære pulje (i området `10.20.1.135` - `10.20.1.175`), subnetmaske `255.255.255.192`, gateway `10.20.1.129` (HSRP Virtual IP) og DNS-server `10.20.1.195`.
...
* **Forventning:** Klienter på `Switch0` port `Fa0/5` lander i VLAN 10 (Administration). Klienter på `Switch1` port `Fa0/5` lander i VLAN 20 (Produktion).
...
* **Beskrivelse:** En pc i Administrationen (VLAN 10, tilsluttet `Switch0`) pinger en pc i IT (VLAN 30, tilsluttet `Switch2`).
* **Forventning:** Da der ikke er lagt nogen ACL-spærring mellem interne klientnetværk (VLAN 10 og 30), skal ICMP-trafikken route fejlfrit gennem Core Switchen (`Multilayer Switch0`).
...
* **Beskrivelse:** En pc på Gæstenetværket (VLAN 40, tilsluttet `Switch3`) forsøger at pinge en computer i Administrationen (VLAN 10, tilsluttet `Switch0`).
...
* **Testmetode (fra Gæste-PC):**
  ...
  ping 10.20.1.228 (ds-01 Management IP) -> Skal fejle (ACL blokerer)
...
* **Beskrivelse:** En IT-medarbejder (VLAN 30, tilsluttet `Switch2`) forsøger at starte en SSH-forbindelse til `Multilayer Switch0`'s management IP (`10.20.1.226`).
...
* **Beskrivelse:** En uautoriseret medarbejder i Administrationen (VLAN 10, tilsluttet `Switch0`) forsøger at tilgå en switch i Management-netværket via SSH.
...
* **Beskrivelse:** Godkendte enheder med registrerede MAC-adresser sender trafik på en sikret port i Administrationen (`Switch0` port `Fa0/1`).
* **Forventning:** Switchen lærer MAC-adressen som `sticky` og gemmer den i running-config. Trafikken flyder normalt.
* **Testmetode (på switch):**
  ```ios
  Switch0# show port-security interface FastEthernet0/1
  ```
...
* **Beskrivelse:** En medarbejder trækker netværkskablet ud af sin godkendte PC og sætter det i sin private bærbare computer på en port i Produktion (`Switch1` port `Fa0/1`, hvor max MAC = 1).
...
* **Beskrivelse:** En bruger medbringer een lille hjemmeswitch og slutter den til et netværksstik i Administrationen (port `Fa0/5` på `Switch0`).
* **Forventning:** Den tilsluttede switch udsender STP BPDU'er. Da BPDU Guard er aktiv på alle access-porte, vil `Switch0` registrere disse pakker og øjeblikkeligt lukke porten (`err-disable`) for at forhindre loops og STP-manipulation.
* **Testmetode (på switch):**
  ```ios
  Switch0# show interface FastEthernet0/5 status
  ```
...
* **Forventning:** `Multilayer Switch0` skal være STP Root for VLAN 10, 20, 30 og 99. `Multilayer Switch1` skal være STP Root for VLAN 40 (Gæster).
* **Testmetode (på Core-switche):**
  ```ios
  Multilayer Switch0# show spanning-tree vlan 10
  ```
  *(Skal vise: "This bridge is the root")*
  ```ios
  Multilayer Switch0# show spanning-tree vlan 40
  ```
  *(Skal vise MAC-adressen på MS1 som Root Bridge)*
  ```ios
  Multilayer Switch1# show spanning-tree vlan 40
  ```
  *(Skal vise: "This bridge is the root")*
* **Status:** Bestået.
...
* **Testmetode (på Core Switch):**
  ```ios
  Multilayer Switch0# show etherchannel summary
  ```
...
* **Beskrivelse:** Vi simulerer et kabelbrud. Mens en pc i Administrationen pinger gatewayen (`10.20.1.129`), afbrydes den ene af de redundante links (f.eks. linket til `Multilayer Switch2`).
* **Forventning:** 
  1. Spanning Tree (Rapid PVST+) registrerer tabet af det aktive link.
  2. Backup-linket til `Multilayer Switch3` skifter fra `Blocking` til `Forwarding` tilstand på under 2 sekunder.
...
* **Beskrivelse:** Vi simulerer et totalt strømudfald eller nedbrud på den primære switch og DHCP-server `Multilayer Switch0`. Derefter tilsluttes en ny PC, som anmoder om en IP-konfiguration.
* **Forventning:**
  1. `Multilayer Switch1` registrerer, at `Multilayer Switch0` er offline via HSRP og overtager rollen som **HSRP Active Gateway** for alle VLANs.
  2. Når den nye PC anmoder om en IP-adresse via DHCP broadcast, vil kun `Multilayer Switch1` være online til at besvare forespørgslen.
  3. `Multilayer Switch1` tildeler med succes en IP-adresse fra sin **20% backup-pool** (f.eks. i området `10.20.1.176` - `10.20.1.190` for VLAN 10).
...
* **Testmetode:**
  1. Klik på `Multilayer Switch0` i Packet Tracer, gå til CLI og kør `shutdown` på alle interfaces (eller slet switchen/træk strømmen i den fysiske visning).
  2. Tilslut en ny tom PC til `Switch0`.
  ...
  *Bekræft at PC'en modtager en IP-adresse i backup-området (f.eks. 10.20.1.176) og den korrekte gateway-adresse (10.20.1.129).*
  5. Ping din router gateway (`10.20.254.1` eller `10.20.254.5`) eller internettet for at bekræfte, at failover routing fungerer 100% perfekt igennem `Multilayer Switch1`!
* **Status:** Bestået.
...
* **Forventning:**
  1. Router `Router0` skal have to aktive OSPF-naboforbindelser (til `2.2.2.1` og `2.2.2.2`) i status **`FULL`**.
  2. `Multilayer Switch0` og `Multilayer Switch1` skal modtage en dynamisk standardrute (`O*E2 0.0.0.0/0`) peget mod `Router0`'s transit-IP'er.
  3. Router `Router0` skal modtage de interne SVI subnets (VLANs) dynamisk som OSPF-ruter (`O 10.20.1.0/25` osv.) fra switchene.
* **Testmetode (på Router Router0):**
  ```ios
...
* **Testmetode (på Multilayer Switch0):**
  ```ios
  Multilayer Switch0# show ip route ospf
  ```
