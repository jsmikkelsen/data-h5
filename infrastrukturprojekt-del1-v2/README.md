...
Designet er blevet yderligere optimeret, så din **Cisco 4331 WAN-router** er **forbundet direkte** til de to FortiGates' `wan1` interfaces. For at understøtte FortiGate HA clustering, kører vi Layer 2 software-bridging (**BDI - Bridge Domain Interface**) direkte på Cisco-routeren.
...
1. **[Netværksdesign og Redundans (`design_valg.md`)](./design_valg.md)**
   ...
   * Edge-sikkerhed og redundans via **FortiGate HA Clustering (FGCP Active/Passive)** med direkte WAN-kabling.
...
3. **[Konfigurationsskabeloner (`konfiguration_skabelon.md`)](./konfiguration_skabelon.md)**
   * Produktionsklare, udeladelsesfrie og kommenterede konfigurationer til alle enheder:
     * **`core-sw01` & `core-sw02` (Cisco 3650):** Gateways, VRF'er, HSRP og statisk leaking.
     * **`acc-sw01` (Cisco 2960X):** LACP EtherChannels, VLAN trunks og access-porte.
     * **`fg-ha` (FortiGate 60F HA Cluster):** HA clustering på FortiLink-porte `a` & `b` samt policies.
     * **`wan-rt01` (Cisco 4331):** BDI-bridging og fuld NAT-opsætning mod rigtigt internet.
...
5. **[Fysisk Kablingsplan (`kablings_plan.md`)](./kablings_plan.md)**
   * Komplet kablingsplan dækkende alle direkte forbindelser mellem Dell R630, FortiGates, L3/L2 switche og WAN-routeren.
