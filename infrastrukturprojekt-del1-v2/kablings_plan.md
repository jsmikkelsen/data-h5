...
*   Dette omdanner routerens to hosliggende og fysisk grupperede porte, **`GigabitEthernet0/0/0`** og **`GigabitEthernet0/0/1`**, til en integreret software-switch.
*   `Gi0/0/0` kables direkte til `fg-ha-01` (`wan1`), og `Gi0/0/1` kables direkte til `fg-ha-02` (`wan1`).
*   Den selvstændige port **`GigabitEthernet0/0/2`** kables direkte til dit rigtige internet (skolens/hjemmets netværk) for at levere internetadgang via NAT.
...
| **wan-rt01** (4331) | `Gi0/0/1` | **fg-ha-02** (FG60F) | `wan1` | Cat6 RJ45 (Grå) | Direkte WAN Link 2 | Direkte WAN link til sekundær firewall (BDI bridged på router) |
...
*   ⚙️ **Grå kabler:** Direkte WAN-forbindelser (mellem router og firewalls samt internet uplink).
 Josephson / Netic NetBox Standard.
