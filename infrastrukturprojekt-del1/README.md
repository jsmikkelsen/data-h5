# Infrastrukturprojekt – Del 1: Fælles Netværksplatform (Enterprise Design med VRF & Firewalls)

Velkommen til projektportfoliet for **Infrastrukturprojekt – Del 1: Netværksplatform**. Dette projekt danner det grundlæggende, skalerbare og redundante fundament for en virksomheds fælles netværksplatform, som på en sikker måde understøtter flere logisk adskilte kundemiljøer samt et lukket administrationsnetværk.
...
1. **[Netværksdesign og Redundans (`design_valg.md`)](./design_valg.md)**
   ...
   * **VRF-Lite & Route Leaking Design:** Detaljeret arkitektur for logisk adskillelse af Kunde A, B, C, D og Management samt kontrolleret route-leaking til Global Routing Table (GRT).

2. **[IP- og VLAN-Plan (`ip_plan.md`)](./ip_plan.md)**
   * Struktureret IP-adresseplan med brug af private IPv4-adresser tilpasset fremtidig vækst.
...
* **Logisk Adskillelse (VRF-Lite):** Hver kunde placeres i sin egen VRF (Virtual Routing and Forwarding) kontekst på Core-switchene (`VRF_A`, `VRF_B`, `VRF_C`, `VRF_D`). En kunde kan under ingen omstændigheder se eller kommunikere med andre kunders routingtabeller, medmindre der specifikt opsættes route-leaking.
