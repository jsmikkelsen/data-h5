# Cisco IOS-XE & FortiOS Konfigurationsskabeloner

Dette dokument indeholder komplette, produktionsklare og grundigt kommenterede konfigurationsskabeloner for alle enheder i netværksplatformen. Alle parametre (IP-adresser, VLAN-numre, HSRP-prioriteter og VRF-kontekster) stemmer 100% overens med den godkendte IP- og VLAN-plan.

---

## 1. core-sw01 (Cisco Catalyst 3650 - Primær Core)

Denne switch agerer Active Gateway for **VLAN 10 (Kunde A)** og **VLAN 20 (Kunde B)** samt **VLAN 99 (Management)**, og Standby Gateway for VLAN 30 og 40.
...
interface Vlan101
 ...
 ip address 10.255.101.2 255.255.255.248
 standby version 2
 standby 101 ip 10.255.101.4
...
router bgp 65001
 ...
 neighbor 10.255.255.2 description Peering med core-sw02
 !
 address-family ipv4
  neighbor 10.255.255.2 activate
  network 10.255.101.0 mask 255.255.255.248
  ! Default route i GRT peger mod FortiGate Edge
  ip route 0.0.0.0 0.0.0.0 10.255.101.1
 exit-address-family
 !
 ...
 exit-address-family
 !
 address-family ipv4 vrf VRF_D
  redistribute connected
 exit-address-family
!
! --- ALT. STATISK ROUTE LEAKING SNIPPET (Anvendes hvis BGP fravælges) ---
! ip route vrf VRF_A 0.0.0.0 0.0.0.0 Vlan101 10.255.101.1 global
! ip route vrf VRF_B 0.0.0.0 0.0.0.0 Vlan101 10.255.101.1 global
! ip route vrf VRF_C 0.0.0.0 0.0.0.0 Vlan101 10.255.101.1 global
! ip route vrf VRF_D 0.0.0.0 0.0.0.0 Vlan101 10.255.101.1 global
! ip route 10.10.0.0 255.255.255.0 Vlan10 10.10.0.10 vrf VRF_A
! ip route 10.20.0.0 255.255.255.0 Vlan20 10.20.0.10 vrf VRF_B
! ip route 10.30.0.0 255.255.255.0 Vlan30 10.30.0.10 vrf VRF_C
! ip route 10.40.0.0 255.255.255.0 Vlan40 10.40.0.10 vrf VRF_D
```
...
Denne switch agerer Active Gateway for **VLAN 30 (Kunde C)** og **VLAN 40 (Kunde D)**, og Standby Gateway for VLAN 10, 20 og 99.
...
interface Vlan101
 description Forbindelse mod FortiGate Edge Cluster
 ip address 10.255.101.3 255.255.255.248
 standby version 2
 standby 101 ip 10.255.101.4
...
router bgp 65001
 ...
 neighbor 10.255.255.1 description Peering med core-sw01
 !
 address-family ipv4
...
interface GigabitEthernet0/1
 description Forbindelse til Kunde A Klient
...
interface GigabitEthernet0/2
 description Forbindelse til Kunde B Klient
...
interface GigabitEthernet0/3
 description Forbindelse til Kunde C Klient
...
interface GigabitEthernet0/4
 description Forbindelse til Kunde D Klient
...
config router static
    edit 1
        ...
        set gateway 10.255.200.2
        set device "wan1"
        set comment "Default Route mod ISP (Cisco 4331)"
    next
    edit 2
        set dst 10.0.0.0 255.0.0.0
        set gateway 10.255.101.4       # Peger på Core-switchenes HSRP VIP i GRT
        set device "port4"
...
config firewall policy
    # Tillad Kunde A internetadgang
    edit 10
        set name "Kunde-A-to-Internet"
        set srcintf "port4"
        set dstintf "wan1"
        set srcaddr "Kunde-A-LAN"
        set dstaddr "all"
        set action accept
        set schedule "always"
        set service "ALL"
    ...
    next
    # Tillad Kunde B internetadgang
    edit 20
        set name "Kunde-B-to-Internet"
        set srcintf "port4"
        set dstintf "wan1"
        set srcaddr "Kunde-B-LAN"
        set dstaddr "all"
        set action accept
        set schedule "always"
        set service "ALL"
...
interface GigabitEthernet0/0/0
 ...
 ip address 10.255.200.2 255.255.255.252
 no shutdown
!
interface Loopback0
...
ip route 10.0.0.0 255.0.0.0 10.255.200.1
```
